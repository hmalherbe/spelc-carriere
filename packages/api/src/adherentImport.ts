...
export async function importAdherentRecords(records: AdherentRecord[]): Promise<ImportAdherentRecordsResult> {
  ...
  // even auto-confirmed/pending link from a previous import is never re-litigated here. Whether an
  // adherent is actually due this campaign (CCMA/CCMI-eligible) is filtered downstream, at display
  // time, against a specific campagne's period — see adherentEligibility.ts and its use in
  // routes/matches.ts and routes/adherents.ts — not here, since matching itself is campagne-agnostic.
