...
| Bloc | État |
|---|---|
| **`packages/domain`** — moteur de calcul (grille indiciaire, dates de promotion, gain brut/net, seuil BA, éligibilité BA/HC/Classe Exceptionnelle, barèmes HC/EXC historiques, reclassements) | ✅ Codé, testé (36 tests), typé strict. Validé contre une vraie valeur observée dans le classeur (148 € de gain net, échelon 5→6 grille AGR). Grille et valeur du point paramétrables (valeurs par défaut intégrées, ou valeurs "live" depuis la base — voir `packages/api/src/liveGrilles.ts`). |
| **`packages/api`** — modèle de données + backend Express/Prisma/JWT | ✅ Schéma validé, migrations appliquées, seed de démo, routes auth/campagnes/teachers/matches/ba-seuils/grilles testées de bout en bout (curl + RBAC). |
| **`packages/web`** — frontend React | ✅ Connexion, tableau de bord enseignants (échelon, date d'accès, gain, éligibilité BA, filtres et tri), file de révision des rapprochements, écran de verrouillage des seuils BA, écran d'import (PDF rectorat multi-fichiers + mise à jour ADEL), écran de mailing (sélection, aperçu, envoi via Brevo), onglet "Règles de gestion" (BA/HC/Classe Exceptionnelle) et onglet "Grilles indiciaires" (tableaux par grade/échelle, indices et valeur du point modifiables par un admin). Vérifié en navigateur (captures d'écran). |
